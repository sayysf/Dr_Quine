...
STR(M(if(!i) return (1);char name[0xff]; char pname[0xff];char cmd[0xfff];sprintf(name,NAME, i - 1);sprintf(pname,PNAME, i - 1);sprintf(cmd,CC,name, pname,pname);int fd = open(name, O_TRUNC | O_CREAT | O_RDWR, 0777);dup2(fd, 1);close(fd);fprintf(stdout,STRING,10,10,10,10,10,i-1,10,34,34,10,34,34,10,34,34,10,10,34,STRING,34,10,s,10);fflush(stdout);system(cmd);))
